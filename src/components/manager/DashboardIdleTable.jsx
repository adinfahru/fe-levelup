...
export default function DashboardIdleTable({ data = [], managerId, onToggleStatus }) {
  ...
  const filtered = useMemo(() => {
    return data.filter(item => {
      ...
      const matchSearch = fullName.includes(search.toLowerCase()) || item.email?.toLowerCase().includes(search.toLowerCase());
  ...
  return (
    <div className="space-y-4 p-4 border rounded-xl bg-white shadow-sm">
        ...
        <TableBody>
          {paginated.map(user => (
            <TableRow key={user.id}>
              ...
              <TableCell>
                <span className={`px-2 py-1 rounded text-xs ${user.status === "Idle" ? "bg-green-100 text-green-700" : "bg-blue-100 text-blue-700"}`}>
                  {user.status}
                </span>
              </TableCell>
      ...
      <div className="flex justify-between">
        <p className="text-sm">Showing {paginated.length} of {filtered.length}</p>
